...
public class AuthController {
    ...
    public ResponseEntity<MensajeResponse> verificarCuenta(
    ...
    public ResponseEntity<MensajeResponse> solicitarRecuperacion(
        ...
        return ResponseEntity.ok(
                new MensajeResponse("Si el correo existe, se enviará un enlace de recuperación."));
    }

    @PostMapping("/restablecer-clave/confirmar")
    public ResponseEntity<MensajeResponse> confirmarRestablecer(
            @RequestBody Map<String, String> request) {
        usuarioService.cambiarClaveConToken(
                request.get("token"),
                request.get("nuevaClave"));
        return ResponseEntity.ok(
                new MensajeResponse("Contraseña restablecida de manera exitosa."));
    }
}