...
function VerificarContenido() {
  ...
  const [correoParaReenviar, setCorreoParaReenviar] = useState<string | null>(
    null
  );
  const [status, setStatus] = useState<
    "loading" | "success" | "error" | "sending"
  >("loading");
      ...
      {(status === "loading" || status === "sending") && (
        <div className="flex flex-col items-center gap-3">
          ...
          <h2 className="text-xl font-bold">
            {status === "sending" ? "Enviando enlace..." : "Verificando cuenta"}
          </h2>
      ...
      {status === "error" && (
        <div className="flex flex-col items-center gap-3 w-full animate-in fade-in duration-500">
          ...
          <div className="rounded-xl bg-red-50 dark:bg-red-900/40 border border-red-200 dark:border-red-500/50 p-4 w-full">
            <p className="text-sm text-red-700 dark:text-red-100 font-medium">
              {mensaje}
            </p>
          </div>
          ...
          {correoParaReenviar && (
            <button
              ...
              className="flex items-center gap-2 text-sm text-[hsl(174_72%_40%)] hover:underline font-medium mt-2 cursor-pointer"
            >
          ...
          <Link
            href="/login"
            className="text-sm text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 underline mt-2"
          >
            Volver al inicio
          </Link>
        </div>
...
export default function VerificarCuentaPage() {
  return (
    <Suspense
      fallback={<Loader2 size={32} className="animate-spin text-slate-400" />}
    >
      <VerificarContenido />
    </Suspense>
  );
